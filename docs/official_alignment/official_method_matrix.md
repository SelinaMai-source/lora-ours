...
- status: official `train.py --help` passes in faithful legacy Python 3.7 / torch 1.4 env, SGD-only preprocess smoke passes, SGD/Taskmaster/MultiWOZ source archives are downloaded and zip-validated, full 37-domain data-loader smoke passes, and a bounded VANILLA/NLG method smoke passes. Full training/reproduction remains blocked by paper-number reproduction for the strongest method.
...
| AdapterCL / ADAPTER | ToDCL README NLG Modularized BLEU `21.7719`, EER `0.163975`; E2E BLEU `16.5768`, EER `0.331949` | yes via ToDCL repo | `e70c1...` / ToDCL | yes; all source archives validated and laid out | bounded method smoke through VANILLA path | same legacy help + full loader + bounded method smoke infrastructure | no | downloaded_not_runnable | likely best non-Multi ToDCL NLG baseline; needs ADAPTER-specific official reproduction/anchor. |
| Replay / REPLAY | ToDCL README NLG Modularized BLEU `21.4832`, EER `0.0559855`; E2E BLEU `16.2668`, EER `0.190309` | yes via ToDCL repo | `e70c1...` / ToDCL | yes; all source archives validated and laid out | bounded method smoke through VANILLA path | same legacy help + full loader + bounded method smoke infrastructure | no | downloaded_not_runnable | strong EER; needs REPLAY-specific official reproduction/anchor. |
...
- ToDCL legacy help smoke, SGD-only preprocess smoke, full 37-domain data-loader smoke, and bounded VANILLA/NLG method smoke pass. ToDCL remains blocked by no official-scale strongest-method reproduction/anchor.
...
3. ToDCL is partially unblocked: faithful legacy env exists, all data archives are laid out, MultiWOZ conversion completed, full 37-domain data-loader smoke passes, and bounded VANILLA/NLG method smoke passes. Full gate remains blocked by official-scale strongest-method reproduction/anchor.

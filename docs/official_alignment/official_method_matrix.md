...
- status: downloaded clean; official `train.py --help` passes in faithful legacy Python 3.7 / torch 1.4 env, SGD-only preprocess smoke passes, SGD/Taskmaster/MultiWOZ source archives are downloaded and zip-validated, and the full 37-domain data-loader smoke passes. Full training/reproduction remains blocked by method-level smoke and paper-number reproduction.
...
- data smoke: ToDCL `get_datasets(dataset_list=['SGD'], develop=True)` passes with train/dev/test totals `532/78/158` over `10` domains. Full ToDCL `get_datasets(dataset_list=['TM19','TM20','MWOZ','SGD'], setting='single', develop=False)` passes with train/dev/test totals `31425/4035/4742` after filtering and BYDOMAIN counts `37/37/37`.
...
| LAMOL | ToDCL README NLG Modularized BLEU `3.49649`, EER `0.35664`; E2E BLEU `3.55622`, EER `0.638889` | yes via ToDCL repo | `e70c1...` / ToDCL | yes; all source archives validated and laid out | full data-loader smoke | `train.py --help` passes; full 37-domain loader passes with train/dev/test `31425/4035/4742` and BYDOMAIN `37/37/37` | no | downloaded_not_runnable | needs method-level smoke before reproduction. |
| AdapterCL / ADAPTER | ToDCL README NLG Modularized BLEU `21.7719`, EER `0.163975`; E2E BLEU `16.5768`, EER `0.331949` | yes via ToDCL repo | `e70c1...` / ToDCL | yes; all source archives validated and laid out | full data-loader smoke | same legacy help + full 37-domain loader smoke | no | downloaded_not_runnable | likely best non-Multi ToDCL NLG baseline, but method-level smoke/reproduction is still blocked. |
| Replay / REPLAY | ToDCL README NLG Modularized BLEU `21.4832`, EER `0.0559855`; E2E BLEU `16.2668`, EER `0.190309` | yes via ToDCL repo | `e70c1...` / ToDCL | yes; all source archives validated and laid out | full data-loader smoke | same legacy help + full 37-domain loader smoke | no | downloaded_not_runnable | strong EER; needs method-level smoke before reproduction. |
| Multi upper bound | ToDCL README NLG Modularized BLEU `26.1462`, EER `0.0341823`; E2E BLEU `23.6073`, EER `0.12558` | yes via ToDCL repo | `e70c1...` / ToDCL | yes; all source archives validated and laid out | full data-loader smoke | same legacy help + full 37-domain loader smoke | no | downloaded_not_runnable | upper bound, not deployed CL base; method-level smoke still required. |
| VANILLA/L2/EWC/AGEM | ToDCL README includes low NLG BLEU/EER baselines | yes via ToDCL repo | `e70c1...` / ToDCL | yes; all source archives validated and laid out | full data-loader smoke | same legacy help + full 37-domain loader smoke | no | downloaded_not_runnable | not best bases; still matrix candidates for comparison. |

Current Dialogue decision:
- ARPER is the only runnable official anchor.
- ToDCL legacy help smoke, SGD-only preprocess smoke, and full 37-domain data-loader smoke pass. ToDCL remains blocked by no method-level smoke/reproduction.
...
3. ToDCL is partially unblocked: faithful legacy env exists, all data archives are laid out, MultiWOZ conversion completed, and the full 37-domain data-loader smoke passes. Full gate remains blocked by method-level smoke/reproduction.

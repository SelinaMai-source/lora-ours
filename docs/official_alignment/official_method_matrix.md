...
- status: official `train.py --help` passes in faithful legacy Python 3.7 / torch 1.4 env, SGD-only preprocess smoke passes, SGD/Taskmaster/MultiWOZ source archives are downloaded and zip-validated, full 37-domain data-loader smoke passes, and bounded VANILLA/ADAPTER/REPLAY NLG method smokes pass. Full training/reproduction remains blocked by paper-number reproduction for the strongest method.
...
- data/method smoke: ToDCL `get_datasets(dataset_list=['SGD'], develop=True)` passes with train/dev/test totals `532/78/158` over `10` domains. Full ToDCL `get_datasets(dataset_list=['TM19','TM20','MWOZ','SGD'], setting='single', develop=False)` passes with train/dev/test totals `31425/4035/4742` after filtering and BYDOMAIN counts `37/37/37`. Bounded VANILLA/ADAPTER/REPLAY NLG method smokes pass on SGD debug; ADAPTER covers `GPT2Adapter`, REPLAY covers replay memory on a second task.
...
| LAMOL | ToDCL README NLG Modularized BLEU `3.49649`, EER `0.35664`; E2E BLEU `3.55622`, EER `0.638889` | yes via ToDCL repo | `e70c1...` / ToDCL | yes; all source archives validated and laid out | bounded method smoke through VANILLA path | `train.py --help`, full 37-domain loader, and bounded VANILLA/NLG method smoke pass | no | downloaded_not_runnable | method-specific smoke/reproduction still pending. |
| AdapterCL / ADAPTER | ToDCL README NLG Modularized BLEU `21.7719`, EER `0.163975`; E2E BLEU `16.5768`, EER `0.331949` | yes via ToDCL repo | `e70c1...` / ToDCL | yes; all source archives validated and laid out | bounded ADAPTER/NLG method smoke | ADAPTER/NLG bounded smoke passed: `GPT2Adapter`, checkpoint reload/save, generated response, scorer entry | no | bounded_method_smoke_passed | best non-MULTI NLG BLEU base; needs full official-equivalent ADAPTER reproduction/anchor. |
| Replay / REPLAY | ToDCL README NLG Modularized BLEU `21.4832`, EER `0.0559855`; E2E BLEU `16.2668`, EER `0.190309` | yes via ToDCL repo | `e70c1...` / ToDCL | yes; all source archives validated and laid out | bounded REPLAY/NLG method smoke | REPLAY/NLG bounded smoke passed over two tasks; second task shows `Memory Size 1`, checkpoint reload/save, generated response, scorer entry | no | bounded_method_smoke_passed | best non-MULTI NLG EER base; needs full official-equivalent REPLAY reproduction/anchor. |
| Multi upper bound | ToDCL README NLG Modularized BLEU `26.1462`, EER `0.0341823`; E2E BLEU `23.6073`, EER `0.12558` | yes via ToDCL repo | `e70c1...` / ToDCL | yes; all source archives validated and laid out | bounded method smoke through VANILLA path | same legacy help + full loader + bounded method smoke infrastructure | no | downloaded_not_runnable | upper bound, not deployed CL base; reproduction useful as ceiling. |
| VANILLA/L2/EWC/AGEM | ToDCL README includes low NLG BLEU/EER baselines | yes via ToDCL repo | `e70c1...` / ToDCL | yes; all source archives validated and laid out | bounded VANILLA/NLG method smoke | VANILLA/NLG bounded smoke passed: checkpoint reload, save, generated response, scorer entry | no | bounded_method_smoke_passed | not a best base; useful infrastructure gate only. |

Current Dialogue decision:
- ARPER is the only runnable official anchor.
- ToDCL legacy help smoke, SGD-only preprocess smoke, full 37-domain data-loader smoke, and bounded VANILLA/ADAPTER/REPLAY NLG method smokes pass. ToDCL remains blocked by no official-scale strongest-method reproduction/anchor.
...
3. ToDCL is partially unblocked: faithful legacy env exists, all data archives are laid out, MultiWOZ conversion completed, full 37-domain data-loader smoke passes, and bounded VANILLA/ADAPTER/REPLAY NLG method smokes pass. Full gate remains blocked by official-scale strongest-method reproduction/anchor.

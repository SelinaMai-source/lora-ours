...
with torch.no_grad():
    ...
    print("Encoder outputs mean:", encoder_outputs.last_hidden_state.mean().item())
    print("Encoder outputs std:", encoder_outputs.last_hidden_state.std().item())
    print("Encoder outputs min:", encoder_outputs.last_hidden_state.min().item())
    print("Encoder outputs max:", encoder_outputs.last_hidden_state.max().item())
    ...
    print("\nDecoder outputs mean:", decoder_outputs.last_hidden_state.mean().item())
    print("Decoder outputs std:", decoder_outputs.last_hidden_state.std().item())
    print("Decoder outputs min:", decoder_outputs.last_hidden_state.min().item())
    print("Decoder outputs max:", decoder_outputs.last_hidden_state.max().item())
    
    lm_logits = model.lm_head(decoder_outputs.last_hidden_state)
    print("\nLM logits mean:", lm_logits.mean().item())
    print("LM logits std:", lm_logits.std().item())
    print("LM logits min:", lm_logits.min().item())
    print("LM logits max:", lm_logits.max().item())

...
print("Let's check the base model's generation on the dataset with max_new_tokens=64 and without any other kwargs, but skip_special_tokens=True, and with a different prompt format, and with max_length=512, and with do_sample=True, and with top_k=50, and with temperature=0.7, and with num_return_sequences=5, and with a different prompt.")

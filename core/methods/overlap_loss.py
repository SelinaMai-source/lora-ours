...
def _resolve_training_device(
    *,
    ...
    """Pick a single device for anti-overlap tensors (prefer CUDA over CPU)."""
    import torch

    devices: List[torch.device] = []
    for acts in activations_by_branch.values():
        if isinstance(acts, torch.Tensor):
            devices.append(acts.device)
    if lora_wrapper is not None:
        for attr in ("device",):
            dev = getattr(lora_wrapper, attr, None)
            if isinstance(dev, torch.device):
                devices.append(dev)
            elif isinstance(dev, str) and dev:
                devices.append(torch.device(dev))
        model = getattr(lora_wrapper, "model", None)
...
def _align_activation_tensors(
    ...
    for branch, acts in activations_by_branch.items():
        if isinstance(acts, torch.Tensor):
            aligned[branch] = acts.to(device)
        else:
            aligned[branch] = acts
    return aligned


def _orthogonal_similarity_from_cfg(cfg: Dict[str, Any]) -> tuple[str, float]:
    ortho_cfg = cfg.get("orthogonal_regularization")
    if not isinstance(ortho_cfg, dict):
        ortho_cfg = {}
    kl_mode = str(ortho_cfg.get("kl_mode", cfg.get("kl_mode", ""))).strip().lower()
    if kl_mode == "matched":
        lambda_scale = float(ortho_cfg.get("kl_bank_lambda", cfg.get("kl_bank_lambda", 1.0)))
        return "information_bottleneck", lambda_scale
...
def compute_overlap_loss_torch(
    *,
    ...
    activations_by_branch = _align_activation_tensors(activations_by_branch, device)
...
def compute_routing_aware_orthogonal_loss(
    *,
    ...
    zero_device = device
    if zero_device is None and lora_wrapper is not None:
        zero_device = _resolve_training_device(activations_by_branch={}, lora_wrapper=lora_wrapper)

    if beta <= 0 or lora_wrapper is None or not hasattr(lora_wrapper, "get_adapter_vector"):
        return torch.tensor(0.0, dtype=torch.float32, device=zero_device)
    ...
    if device is None:
        device = _resolve_training_device(activations_by_branch={}, lora_wrapper=lora_wrapper)
    if device is None:
        device = next(iter(vectors.values())).device
    vectors = {b: v.to(device) for b, v in vectors.items()}
    total = torch.tensor(0.0, device=device)
    count = 0
...
def compute_anti_overlap_training_loss(
    *,
    ...
    if device is not None and activations_by_branch:
        activations_by_branch = _align_activation_tensors(activations_by_branch, device)
    ...
    weight_similarity, kl_scale = _orthogonal_similarity_from_cfg(cfg)
    if betas["weight_beta"] > 0:
        weight_loss = compute_orthogonal_weight_loss(
            lora_wrapper=lora_wrapper,
            beta=betas["weight_beta"] * kl_scale,

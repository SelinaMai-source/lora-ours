...
print("Let's check if the model is producing valid logits for a different prompt.")
...
print("Let's see what happens if we feed the top token back in.")
decoder_input_ids = torch.tensor([[model.config.decoder_start_token_id, 10465]]) # 10465 is 'Bookmark'
with torch.no_grad():
    outputs = model(
        input_ids=inputs.input_ids,
        attention_mask=inputs.attention_mask,

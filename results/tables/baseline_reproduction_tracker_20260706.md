...
| Replay(50) | **40.4** | up to **1.6** | — | — | **queued → launching** (priority **1**) | Smoke only: 50.0 AR on 1 task (`replay50_v55_smoke`); formal 19-task **not started** |
...
| Item | State |
|------|-------|
| GPU owner (blocking) | Standard **v85 Ours overlay smoke** (`run_uie_lora.py` round2 amazon, `max_steps=20`) — not a baseline repro job |
| **Launched now** | Baseline queue waiting for GPU idle, then **CITB Replay(50) formal** → tmux `lora-ours-citb-replay50-formal` |

#!/usr/bin/env bash
# ARPER WOZ3 v1: v89 SCLSTM anchor + SSRG exemplar selection overlay (Path B only).
set -euo pipefail
...
TRAIN_CMD="cd ${REPO_ROOT} && CUDA_VISIBLE_DEVICES=${GPU_ID} PYTHONUNBUFFERED=1 ${PYTHON_BIN} scripts/arper_ssrg_overlay_train_wrapper.py 2>&1 | tee -a ${LOG_PATH}"

...
RUN_NAME = "standard_peft_cl_o_lora_standard_order1_seed1_ours_strict_v64_safe_short_isolated_eval_exit_diagnostic"
CONFIG = "configs/ccfa_three_suite/standard_peft_cl_o_lora_standard_order1_seed1_ours_strict_v64_safe_short_probe.yaml"
...
def launch(config: str, force: bool) -> int:
    LOG_DIR.mkdir(parents=True, exist_ok=True)
    config_path = (REPO / config).resolve() if not Path(config).is_absolute() else Path(config)
    ...
    if existing_train and not force:
        ...
        print("Refusing v64 safe-short launch while core.train is already running.", file=sys.stderr)
        return 75
    ...
    env.update(
        {
            "PYTHONUNBUFFERED": "1",
            "WANDB_MODE": "online",
            "V64_STANDARD_SAFE_SHORT": "1",
        }
    )
    stdout = SUPERVISOR_LOG.open("ab", buffering=0)
    command = ["bash", "scripts/run_ours_v1_strict_iteration.sh", str(config_path)]
    ...
    payload = {
        "updated_at": now(),
        "event": "isolated_safe_short_child_started",
        "run_name": RUN_NAME,
        "config": str(config_path),
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Launch the v64 Standard safe-short diagnostic as an isolated child.")

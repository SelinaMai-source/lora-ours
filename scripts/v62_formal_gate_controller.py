...
def main() -> None:
    print(f"[{now()}] v62 formal gate controller started", flush=True)
    write_decision({"state": "watching_formal", "run_id": EARLY_RUN, "thresholds": PASS_THRESHOLDS})
    while True:
        ...
        if state in {"stopped_low_or_failed", "eval_failed", "low_score_gate", "unknown_not_running", "exited_with_artifact"}:
            ...
            break
        time.sleep(60)
    print(f"[{now()}] v62 formal gate controller exiting", flush=True)

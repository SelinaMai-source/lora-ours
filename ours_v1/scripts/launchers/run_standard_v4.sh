#!/usr/bin/env bash
# Standard ours-v4: O-LoRA + class-coverage SSRG + single delta [replay_budget].
set -euo pipefail
...
if [[ "${FORMAL}" == "1" ]]; then
  RUN_NAME="${RUN_NAME:-olora_official_base_ours_overlay_v4_20260712_formal_order1_seed1}"
  WANDB_GROUP="${WANDB_GROUP:-published-base-standard-olora-plus-ours-overlay-v4-formal}"
  RUN_LABEL="${RUN_LABEL:-published_base_olora_plus_replay_budget_v4_formal}"
else
  RUN_NAME="${RUN_NAME:-olora_official_base_ours_overlay_v4_20260712_smoke_order1_seed1}"
  WANDB_GROUP="${WANDB_GROUP:-published-base-standard-olora-plus-ours-overlay-v4-smoke}"
  RUN_LABEL="${RUN_LABEL:-published_base_olora_plus_replay_budget_v4_smoke}"
fi
...
# Single-mechanism delta from ours-v3 (96 -> 128) for ours-v4:
export REPLAY_PER_TASK=128
export FORMAL
